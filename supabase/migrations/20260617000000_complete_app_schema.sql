...
CREATE TABLE IF NOT EXISTS public.pos_queue (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.internal_stock_usage (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  product_id UUID NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.parent_issues (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.ecd_report_settings (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.installation_payments (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Indexes for admission tables

CREATE INDEX IF NOT EXISTS idx_admission_links_tenant ON public.admission_links(tenant_id);
CREATE INDEX IF NOT EXISTS idx_admission_links_code ON public.admission_links(link_code);
CREATE INDEX IF NOT EXISTS idx_admission_confirmations_tenant ON public.admission_confirmations(tenant_id);
CREATE INDEX IF NOT EXISTS idx_admission_confirmations_code ON public.admission_confirmations(confirmation_code);
CREATE INDEX IF NOT EXISTS idx_admission_confirmations_link ON public.admission_confirmations(admission_link_id);
CREATE INDEX IF NOT EXISTS idx_admission_confirmations_verified ON public.admission_confirmations(verified_at);

-- RLS for admission_links

ALTER TABLE public.admission_links ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS 'Staff can view their tenant admission links'
  ON public.admission_links FOR SELECT
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can insert admission links for their tenant'
  ON public.admission_links FOR INSERT
  WITH CHECK (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can update their tenant admission links'
  ON public.admission_links FOR UPDATE
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can delete their tenant admission links'
  ON public.admission_links FOR DELETE
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

-- RLS for admission_confirmations

ALTER TABLE public.admission_confirmations ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS 'Staff can view their tenant admission confirmations'
  ON public.admission_confirmations FOR SELECT
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can insert admission confirmations'
  ON public.admission_confirmations FOR INSERT
  WITH CHECK (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can update their tenant admission confirmations'
  ON public.admission_confirmations FOR UPDATE
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

-- RLS for admission_settings

ALTER TABLE public.admission_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS 'Staff can view their tenant admission settings'
  ON public.admission_settings FOR SELECT
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can insert their tenant admission settings'
  ON public.admission_settings FOR INSERT
  WITH CHECK (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

CREATE POLICY IF NOT EXISTS 'Staff can update their tenant admission settings'
  ON public.admission_settings FOR UPDATE
  USING (tenant_id IN (SELECT profiles.tenant_id FROM profiles WHERE profiles.id = auth.uid()));

-- Generate confirmation code function

CREATE OR REPLACE FUNCTION public.generate_confirmation_code() RETURNS TEXT
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  code TEXT;
...
BEGIN
  LOOP
    ...
    END LOOP;
    IF NOT EXISTS (SELECT 1 FROM public.admission_confirmations WHERE confirmation_code = code) THEN
      RETURN code;
    END IF;
  END LOOP;
END;
...
DECLARE
  ...
  result JSONB;
BEGIN
  SELECT * INTO link_record FROM public.admission_links
  WHERE link_code::text = p_link_code
  AND tenant_id = p_tenant_id
  AND is_active = true
  FOR UPDATE;
  ...
  IF link_record.expires_at < now() THEN
    UPDATE public.admission_links SET is_active = false WHERE id = link_record.id;
    RETURN jsonb_build_object('success', false, 'error', 'Link has expired');
  END IF;

  IF link_record.registrations_used >= link_record.max_registrations THEN
    RETURN jsonb_build_object('success', false, 'error', 'Maximum registrations reached for this link');
  END IF;

  IF upper(link_record.payment_code) != upper(p_payment_code) THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid payment code');
  END IF;

  UPDATE public.admission_links
  SET registrations_used = registrations_used + 1
...
-- Public can view school's admission settings (limited fields)
CREATE POLICY IF NOT EXISTS 'Public can view open admission settings'
  ON public.admission_settings FOR SELECT
  USING (is_open = true);

-- Public can view active admission links
CREATE POLICY IF NOT EXISTS 'Public can view active admission links'
  ON public.admission_links FOR SELECT
