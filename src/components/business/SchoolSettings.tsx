...
import { Plus, X, Save, Loader2, GraduationCap, Copy, Users, CreditCard, Hash, Shield, BookOpen } from "lucide-react";
...
import { SchoolGoverningBody } from "@/components/school/SchoolGoverningBody";
import { SchoolDepartments } from "@/components/school/SchoolDepartments";
  ...
  return (
    <Tabs defaultValue="general">
      <TabsList className="mb-6">
        <TabsTrigger value="general"><GraduationCap className="h-4 w-4 mr-1" />General</TabsTrigger>
        <TabsTrigger value="governing"><Shield className="h-4 w-4 mr-1" />Governing</TabsTrigger>
        <TabsTrigger value="departments"><BookOpen className="h-4 w-4 mr-1" />Departments</TabsTrigger>
      </TabsList>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => { if (tenantInfo?.parent_login_code) { navigator.clipboard.writeText(tenantInfo.parent_login_code); toast({ title: "Code copied to clipboard" }); }}}>
                        <Copy className="h-4 w-4" />
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground">Parents will enter this code at <span className="font-medium">{window.location.origin}/parent</span></p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-red-500/20 bg-red-500/5">
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  <div className="p-2 bg-red-500/10 rounded-lg"><CreditCard className="h-5 w-5 text-red-500" /></div>
                  <div className="flex-1 space-y-4">
                    <div>
                      <Label className="text-base font-semibold">Fee Balance Threshold</Label>
                      <p className="text-sm text-muted-foreground">Parents will not be able to view report cards if their child's balance exceeds this amount. Set to 0 to disable this restriction.</p>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="feeThreshold">Threshold Amount (UGX)</Label>
                      <Input id="feeThreshold" type="number" value={feeBalanceThreshold} onChange={(e) => setFeeBalanceThreshold(Number(e.target.value))} placeholder="0" min={0} />
                      <p className="text-xs text-muted-foreground">{feeBalanceThreshold > 0 ? `Parents with balances above UGX ${feeBalanceThreshold.toLocaleString()} won't see reports` : "No restriction - all parents can view reports"}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Customize how student IDs appear on ID cards</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="studentIdPrefix">ID Prefix</Label>
                        <Input id="studentIdPrefix" value={studentIdPrefix} onChange={(e) => setStudentIdPrefix(e.target.value.toUpperCase())} placeholder="STU" maxLength={10} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="studentIdDigits">Number Digits</Label>
                        <Select value={String(studentIdDigits)} onValueChange={(v) => setStudentIdDigits(parseInt(v))}>
                    ...
                    <div className="bg-background border rounded-lg px-4 py-3">
                      ...
                      <p className="font-mono text-lg font-bold">{studentIdPrefix}-{"0".repeat(studentIdDigits - 1)}1</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-orange-500/20 bg-orange-500/5">
              <CardContent className="pt-6">
                <div className="flex items-start gap-4">
                  <div className="p-2 bg-orange-500/10 rounded-lg"><Hash className="h-5 w-5 text-orange-500" /></div>
                  <div className="flex-1 space-y-4">
                    <div>
                      <Label className="text-base font-semibold">Admission Number Format</Label>
                      <p className="text-sm text-muted-foreground">Customize how admission numbers are generated</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="admissionPrefix">Prefix</Label>
                        <Input id="admissionPrefix" value={admissionPrefix} onChange={(e) => setAdmissionPrefix(e.target.value.toUpperCase())} placeholder="ADM" maxLength={10} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="admissionFormat">Format Template</Label>
                        <Select value={admissionFormat} onValueChange={setAdmissionFormat}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="bg-background border rounded-lg px-4 py-3">
                      <Label className="text-xs text-muted-foreground">Preview</Label>
                      <p className="font-mono text-lg font-bold">{admissionFormat.replace('{PREFIX}', admissionPrefix).replace('{YY}', '25').replace('{YYYY}', '2025').replace('{NUMBER}', '0001')}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            {isSecondarySchool && <><UNEBSettings tenantId={tenantId} /><Separator /></>}
            {isSchool && tenantId && <><AdmissionSettings tenantId={tenantId} /><Separator /></>}
            {isSchool && tenantId && <><SchoolPaySettings tenantId={tenantId} /><Separator /></>}

            {/* Class Streams */}
            <div className="space-y-4">
              <div>
                <Label>Class Streams</Label>
                <p className="text-sm text-muted-foreground mb-3">Streams are used to divide classes (e.g., Senior 1 East, Senior 1 West)</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {streams.map((stream) => (
                    <Badge key={stream} variant="secondary" className="text-sm py-1 px-3">
                      {stream}
                      <button type="button" onClick={() => removeStream(stream)} className="ml-2 hover:text-destructive"><X className="h-3 w-3" /></button>
                    </Badge>
                  ))}
                  {streams.length === 0 && <p className="text-sm text-muted-foreground">No streams added</p>}
                </div>
                <div className="flex gap-2">
                  <Input value={newStream} onChange={(e) => setNewStream(e.target.value)} placeholder="Add new stream (e.g., Red, Blue)" onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addStream())} />
                  <Button type="button" variant="outline" onClick={addStream}><Plus className="h-4 w-4" /></Button>
                </div>
              </div>
            </div>
      ...
      <TabsContent value="governing">
        <SchoolGoverningBody tenantId={tenantId} />
      </TabsContent>

      <TabsContent value="departments">
        <SchoolDepartments tenantId={tenantId} />
      </TabsContent>
    </Tabs>
  );
}
