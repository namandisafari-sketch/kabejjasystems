...
export function BranchManagement({ tenantId, branchLimit }: BranchManagementProps) {
  ...
  const canAddBranch = (branches?.length || 0) < branchLimit;
  ...
  const saveBranchMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      if (editingBranch) {
        ...
        if (!canAddBranch) {
          throw new Error(`You've reached your branch limit of ${branchLimit}. Upgrade your package to add more branches.`);
        }
        const { error } = await supabase
          .from('branches')
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Manage your business locations ({branches?.length || 0}/{branchLimit} branches used)
            </CardDescription>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={(open) => {
            ...
            <DialogTrigger asChild>
              <Button size="sm" disabled={!canAddBranch && !editingBranch}>
      ...
      <CardContent>
        {!canAddBranch && (
          <Alert className="mb-4">
            ...
            <AlertDescription>
              You've reached your branch limit ({branchLimit} branches). Upgrade your subscription package to add more branches.
            </AlertDescription>
          </Alert>
        )}
              ...
              <TableBody>
                {branches.map((branch) => (
                  <TableRow key={branch.id}>
                    ...
                    <TableCell>
                      <Badge className={branch.is_active ? 'bg-green-500/20 text-green-600' : 'bg-muted'}>
