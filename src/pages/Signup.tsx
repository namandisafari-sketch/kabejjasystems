...
const Signup = () => {
  ...
  // Fetch business packages for non-school, non-rental businesses
  const { data: businessPackages } = useQuery({
    ...
    enabled: !isSchoolType && !isRentalType && !!formData.businessType,
  });

  // Fetch rental packages for rental businesses
  ...
  // Legacy packages query (fallback)
  const { data: packages } = useQuery({
    ...
    enabled: false, // Disabled - using specific package tables now
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSchoolType && formData.schoolPackageId) {
        const { error: schoolDataError } = await db
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                  Up to {pkg.max_properties} properties, {pkg.max_units} units
                                </p>
                                <p className="mt-2">
                                  <span className="text-2xl font-bold text-foreground">
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm text-muted-foreground mt-0.5">{pkg.description}</p>
                                <p className="text-xs text-muted-foreground mt-1">
                                  {pkg.max_branches} branches, {pkg.max_users} users
                                </p>
                                <p className="mt-2">
                                  <span className="text-2xl font-bold text-foreground">
                                    {new Intl.NumberFormat('en-UG', { style: 'currency', currency: 'UGX', maximumFractionDigits: 0 }).format(pkg.monthly_price)}
