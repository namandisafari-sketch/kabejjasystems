...
const AdminTenantDetails = () => {
  ...
  const { data: tenantStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        db.from('profiles').select('id, full_name, role', { count: 'exact' }).eq('tenant_id', id),
        db.from('employees').select('id, full_name, role, is_active', { count: 'exact' }).eq('tenant_id', id),
        db.from('branches').select('id, name, is_active', { count: 'exact' }).eq('tenant_id', id),
        db.from('products').select('id', { count: 'exact' }).eq('tenant_id', id),
        db.from('customers').select('id', { count: 'exact' }).eq('tenant_id', id),
        db.from('sales').select('id, total_amount, payment_status').eq('tenant_id', id),
        db.from('students').select('id, is_active', { count: 'exact' }).eq('tenant_id', id),
      ]);
  ...
  const deleteTenantMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await db.functions.invoke('delete-tenant', {
