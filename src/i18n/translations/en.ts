export const en = {
  // Common
  common: {
    save: "Save",
    cancel: "Cancel",
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    search: "Search...",
    loading: "Loading...",
    noResults: "No results found",
    active: "Active",
    inactive: "Inactive",
    total: "Total",
    actions: "Actions",
    confirm: "Confirm",
    back: "Back",
    next: "Next",
    submit: "Submit",
    close: "Close",
    yes: "Yes",
    no: "No",
    all: "All",
    filter: "Filter",
    export: "Export",
    import: "Import",
    print: "Print",
    download: "Download",
    upload: "Upload",
    refresh: "Refresh",
    view: "View",
    details: "Details",
    status: "Status",
    date: "Date",
    name: "Name",
    email: "Email",
    phone: "Phone",
    address: "Address",
    notes: "Notes",
    description: "Description",
    amount: "Amount",
    quantity: "Quantity",
    price: "Price",
    type: "Type",
    category: "Category",
    error: "Error",
    success: "Success",
    warning: "Warning",
    members: "members",
  },

  // Auth
  auth: {
    login: "Login",
    logout: "Logout",
    signup: "Sign Up",
    email: "Email",
    password: "Password",
    forgotPassword: "Forgot Password?",
    rememberMe: "Remember me",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
  },

  // Navigation
  nav: {
    dashboard: "Dashboard",
    students: "Students",
    classes: "Classes",
    subjects: "Subjects",
    attendance: "Attendance",
    fees: "Fees",
    exams: "Exams",
    examSessions: "Exam Sessions",
    reportCards: "Report Cards",
    grades: "Grades",
    parents: "Parents",
    employees: "Staff",
    payroll: "Payroll",
    timetable: "Timetable",
    discipline: "Discipline",
    settings: "Settings",
    reports: "Reports",
    gateCheckin: "Gate Check-in",
    visitorRegister: "Visitor Register",
    termCalendar: "Term Calendar",
    admissionLinks: "Admission Links",
    admissionConfirmations: "Admission Confirmations",
    unebRegistration: "UNEB Registration",
    unebCandidates: "UNEB Candidates",
    navigation: "Navigation",
    managementPortal: "Management Portal",
    pos: "POS",
    products: "Products",
    sales: "Sales",
    customers: "Customers",
    expenses: "Expenses",
    inventory: "Inventory",
    assets: "Assets",
    letters: "Letters",
    accounting: "Accounting",
    requisitions: "Requisitions",
    studentCards: "Student Cards",
    schoolHolidays: "School Holidays",
    termRequirements: "Term Requirements",
    studentLifecycle: "Student Lifecycle",
    promotionRules: "Promotion Rules",
    marksEntry: "Marks Entry",
    importExamResults: "Import Exam Results",
    examImportPermissions: "Exam Import Permissions",
    blockedExamAccess: "Blocked Exam Access",
    academicAnalytics: "Academic Analytics",
    branches: "Branches",
    qrCodes: "QR Codes",
    businessCards: "Business Cards",
  },

  // Dashboard
  dashboard: {
    title: "Dashboard",
    welcome: "Welcome back",
    overview: "Overview",
    totalStudents: "Total Students",
    totalStaff: "Total Staff",
    totalRevenue: "Total Revenue",
    totalExpenses: "Total Expenses",
    recentActivity: "Recent Activity",
    quickActions: "Quick Actions",
    feesCollected: "Fees Collected",
    feesOutstanding: "Fees Outstanding",
    attendanceRate: "Attendance Rate",
    enrollmentTrend: "Enrollment Trend",
  },

  // Students
  students: {
    title: "Students",
    addStudent: "Add Student",
    editStudent: "Edit Student",
    studentDetails: "Student Details",
    fullName: "Full Name",
    admissionNumber: "Admission Number",
    class: "Class",
    gender: "Gender",
    dateOfBirth: "Date of Birth",
    parentName: "Parent/Guardian Name",
    parentPhone: "Parent Phone",
    enrollmentDate: "Enrollment Date",
    male: "Male",
    female: "Female",
    totalStudents: "Total Students",
    activeStudents: "Active Students",
  },

  // Employees/Staff
  staff: {
    title: "Staff",
    addStaff: "Add Staff Member",
    editStaff: "Edit Staff",
    fullName: "Full Name",
    role: "Role",
    department: "Department",
    salary: "Salary",
    hireDate: "Hire Date",
    active: "Active",
    payroll: "Payroll",
    activate: "Activate",
    deactivate: "Deactivate",
    totalStaff: "Total",
    activeStaff: "Active",
    totalPayroll: "Payroll",
    noStaffFound: "No staff found",
  },

  // Fees
  fees: {
    title: "Fees",
    collectPayment: "Collect Payment",
    feeStructure: "Fee Structure",
    paymentHistory: "Payment History",
    balance: "Balance",
    paid: "Paid",
    pending: "Pending",
    overdue: "Overdue",
    paymentMethod: "Payment Method",
    cash: "Cash",
    bankTransfer: "Bank Transfer",
    mobileMoney: "Mobile Money",
    receipt: "Receipt",
    totalCollected: "Total Collected",
    totalOutstanding: "Total Outstanding",
  },

  // Exams
  exams: {
    title: "Exams",
    createExam: "Create Exam",
    examName: "Exam Name",
    subject: "Subject",
    marks: "Marks",
    grade: "Grade",
    totalMarks: "Total Marks",
    passMark: "Pass Mark",
    results: "Results",
    average: "Average",
  },

  // Attendance
  attendance: {
    title: "Attendance",
    markAttendance: "Mark Attendance",
    present: "Present",
    absent: "Absent",
    late: "Late",
    excused: "Excused",
    attendanceRate: "Attendance Rate",
    today: "Today",
    thisWeek: "This Week",
    thisMonth: "This Month",
  },

  // Classes
  classes: {
    title: "Classes",
    addClass: "Add Class",
    className: "Class Name",
    classTeacher: "Class Teacher",
    totalStudents: "Total Students",
    stream: "Stream",
    section: "Section",
  },

  // Settings
  settings: {
    title: "Settings",
    general: "General",
    school: "School Settings",
    billing: "Billing",
    users: "Users",
    security: "Security",
    notifications: "Notifications",
    language: "Language",
    selectLanguage: "Select Language",
    english: "English",
    arabic: "العربية",
    luganda: "Luganda",
  },

  // Reports
  reports: {
    title: "Reports",
    generate: "Generate Report",
    financialReport: "Financial Report",
    academicReport: "Academic Report",
    attendanceReport: "Attendance Report",
    staffReport: "Staff Report",
  },

  // Parents
  parents: {
    title: "Parents",
    addParent: "Add Parent",
    parentName: "Parent Name",
    children: "Children",
    contactInfo: "Contact Information",
  },

  // Discipline
  discipline: {
    title: "Discipline",
    addCase: "Report Incident",
    caseNumber: "Case Number",
    incidentDate: "Incident Date",
    incidentType: "Incident Type",
    actionTaken: "Action Taken",
    resolved: "Resolved",
    pending: "Pending",
  },

  // Counseling
  counseling: {
    title: "Guidance & Counseling",
    newSession: "New Session",
    sessionNumber: "Session #",
    counselor: "Counselor",
    issueCategory: "Issue Category",
    sessionType: "Session Type",
    activeCases: "Active Cases",
    resolved: "Resolved",
    referred: "Referred",
  },

  // Timetable
  timetable: {
    title: "Timetable",
    monday: "Monday",
    tuesday: "Tuesday",
    wednesday: "Wednesday",
    thursday: "Thursday",
    friday: "Friday",
    saturday: "Saturday",
    period: "Period",
    break: "Break",
  },

  // Gate Check-in
  gate: {
    title: "Gate Check-in",
    scanQR: "Scan QR Code",
    checkIn: "Check In",
    checkOut: "Check Out",
    visitor: "Visitor",
    student: "Student",
  },

  // ECD specific
  ecd: {
    pupils: "Pupils",
    learningAreas: "Learning Areas",
    progress: "Progress",
    roles: "Class Roles",
    pupilCards: "Pupil Cards",
  },

  // HR / Onboarding
  hr: {
    onboarding: "Employee Onboarding",
    employeeDetails: "Employee Details",
    personalInfo: "Personal Information",
    contactInfo: "Contact Information",
    emergencyContact: "Emergency Contact",
    employmentDetails: "Employment Details",
    bankingAndCompliance: "Banking & Compliance",
    qualifications: "Qualifications",
    reviewAndSubmit: "Review & Submit",
    fullName: "Full Name",
    dateOfBirth: "Date of Birth",
    gender: "Gender",
    maritalStatus: "Marital Status",
    nationality: "Nationality",
    nin: "National ID (NIN)",
    bloodGroup: "Blood Group",
    religion: "Religion",
    homeDistrict: "Home District",
    email: "Email Address",
    phone: "Phone Number",
    alternativePhone: "Alternative Phone",
    address: "Physical Address",
    emergencyName: "Emergency Contact Name",
    emergencyRelation: "Relationship",
    emergencyPhone: "Emergency Phone",
    employeeNumber: "Employee Number",
    department: "Department",
    role: "Role / Job Title",
    jobGrade: "Job Grade / Level",
    employmentType: "Employment Type",
    workSchedule: "Work Schedule",
    hireDate: "Start Date",
    contractStart: "Contract Start Date",
    contractEnd: "Contract End Date",
    probationEnd: "Probation End Date",
    salary: "Monthly Salary (UGX)",
    bankName: "Bank Name",
    accountName: "Account Name",
    accountNumber: "Account Number",
    bankBranch: "Bank Branch",
    nssfNumber: "NSSF Number",
    tinNumber: "TIN Number",
    highestEducation: "Highest Education Level",
    yearOfGraduation: "Year of Graduation",
    institution: "Institution",
    certifications: "Professional Certifications",
    skills: "Skills",
    previousEmployer: "Previous Employer",
    previousRole: "Previous Role",
    yearsOfExperience: "Years of Experience",
    permanent: "Permanent",
    contract: "Contract",
    partTime: "Part-time",
    intern: "Intern",
    fullTime: "Full-time",
    partTimeSchedule: "Part-time",
    shiftBased: "Shift-based",
    flexible: "Flexible",
    remote: "Remote",
    single: "Single",
    married: "Married",
    divorced: "Divorced",
    widowed: "Widowed",
    separated: "Separated",
    male: "Male",
    female: "Female",
    other: "Other",
    enterSkill: "Type a skill and press Add",
    addCertification: "Add Certification",
    certificationName: "Certification Name",
    step: "Step",
    of: "of",
    reviewInfo: "Please review all information before submitting",
    submitOnboarding: "Submit & Complete Onboarding",
    onboardingSuccess: "Employee onboarded successfully",
    onboardingComplete: "Onboarding Complete",
  },

  // Pages
  pages: {
    index: {
      title: "TennaHub",
      subtitle: "Business Management Platform",
      company: "TennaHub Technologies",
    },
    login: {
      title: "Welcome Back",
      subtitle: "Sign in to manage your school, track students, and stay on top of everything.",
      backToHome: "Back to home",
      signIn: "Sign In",
      signingIn: "Signing in...",
      emailLabel: "Email",
      emailPlaceholder: "your@email.com",
      passwordLabel: "Password",
      passwordPlaceholder: "Enter your password",
      schoolCodeLabel: "School/Business Code",
      schoolCodePlaceholder: "e.g. STMARYS or ABC123",
      schoolCodeHint: "Enter your school/business code to continue.",
      staffPortalLogin: "Staff Portal Login",
      noAccount: "Don't have an account?",
      signUp: "Sign up",
      loginFailed: "Login Failed",
      accessDenied: "Access Denied",
      subscriptionExpired: "Your school subscription has expired. Please renew to continue.",
      welcomeBack: "Welcome back!",
      loggedInAsAdmin: "You've successfully logged in as admin",
      loggedIn: "You've successfully logged in",
      signInDescription: "Enter your credentials to continue",
    },
    notFound: {
      title: "404",
      message: "Oops! Page not found",
      returnHome: "Return to Home",
    },
    pendingApproval: {
      title: "Payment Under Review",
      description: "Your payment proof has been submitted successfully",
      details: "Our admin team will review your payment within 24-72 hours. You'll receive a notification once your account is activated.",
      whatsNext: "What's Next?",
      steps: [
        "Admin reviews your payment proof",
        "You receive activation notification",
        "Access your full dashboard",
        "Start managing your business",
      ],
      contact: "Need help? Contact us at support@tennahubapps.com",
    },
    subscriptionExpired: {
      trialEnded: "Free Trial Ended",
      subscriptionExpired: "Subscription Expired",
      trialMessage: "Your 14-day free trial has ended. Upgrade now to continue using all features!",
      expiredMessage: "Your business subscription has reached its cycle limit. Please contact the administrator to renew your subscription.",
      readyToContinue: "Ready to Continue?",
      upgradeMessage: "Upgrade to a paid plan to keep all your data and continue managing your business.",
      upgradeNow: "Upgrade Now",
      accessDenied: "All users linked to this business cannot access the system until the subscription is renewed.",
      contactSupport: "Contact Support:",
      emailSupport: "Email Support",
      refresh: "Refresh",
      logOut: "Log Out",
    },
    studentLogin: {
      studentPortal: "Student Portal",
      welcome: "Welcome",
      enterSchoolCode: "Enter your school code to continue",
      signInWithAccount: "Sign in with your student account",
      schoolCodeLabel: "School Code",
      schoolCodePlaceholder: "e.g. KAB001",
      continue: "Continue",
      changeSchool: "Change school",
      emailLabel: "Email",
      emailPlaceholder: "student@example.com",
      passwordLabel: "Password",
      passwordPlaceholder: "\u0007\u0007\u0007\u0007\u0007\u0007\u0007\u0007",
      signIn: "Sign In",
      invalidSchoolCode: "Invalid school code",
      invalidCredentials: "Invalid email or password",
      noStudentRecord: "No student record found for this account",
    },
    studentSetPassword: {
      studentPortal: "Student Portal",
      setPassword: "Set Your Password",
      description: "This is your first login. Please choose a new password to continue.",
      newPasswordLabel: "New Password",
      newPasswordPlaceholder: "Enter new password",
      confirmPasswordLabel: "Confirm Password",
      confirmPasswordPlaceholder: "Confirm new password",
      setPasswordButton: "Set Password",
      allFieldsRequired: "All fields are required.",
      passwordTooShort: "Password must be at least 6 characters.",
      passwordsDoNotMatch: "Passwords do not match.",
    },
    adminSignup: {
      validating: "Validating Invitation...",
      pleaseWait: "Please wait while we verify your access...",
      accessDenied: "Access Denied",
      notAuthorized: "You are not authorized to access this page.",
      returnHome: "Return Home",
      platformAdmin: "Platform Admin",
      createAccount: "Create a platform administrator account",
      fullNameLabel: "Full Name *",
      emailLabel: "Email *",
      passwordLabel: "Password *",
      confirmPasswordLabel: "Confirm Password *",
      adminNamePlaceholder: "Admin Name",
      emailPlaceholder: "admin@example.com",
      passwordPlaceholder: "Min. 8 characters",
      confirmPlaceholder: "Re-enter password",
      createButton: "Create Admin Account",
      businessOwner: "Business owner?",
      signUpHere: "Sign up here",
      alreadyHaveAccount: "Already have an account?",
      login: "Login",
      disabledSignup: "Platform admin signup is currently disabled. Please use an invitation link.",
      invalidToken: "Invalid, expired, or already-used invitation token.",
      tokenRequired: "Admin signup requires a valid invitation token.",
    },
  },

  // Navigation UI
  navigation: {
    businessSidebarCategories: {
      core: "General",
      students: "Students",
      school: "School",
      academics: "Academics",
      restaurant: "Restaurant",
      hotel: "Hotel",
      repair: "Repair",
      salon: "Salon",
      pharmacy: "Pharmacy",
      property: "Properties",
      rental: "Rental",
      finance: "Finance",
      people: "People",
      operations: "Operations",
      communication: "Communication",
      analytics: "Analytics",
      admin: "Admin",
      legal: "Legal",
      hr: "HR",
    },
    adminSidebarCategories: {
      overview: "Overview",
      management: "Management",
      students: "Students",
      billing: "Billing",
      operations: "Operations",
      community: "Community",
      security: "Security",
    },
    adminSidebarItems: {
      dashboard: "Dashboard",
      createBusiness: "Create Business",
      tenants: "Tenants",
      sponsors: "Sponsors",
      partnershipDeed: "Partnership Deed",
      studentAccounts: "Student Accounts",
      withdrawal: "Withdrawal",
      subscriptions: "Subscriptions",
      payments: "Payments",
      packages: "Packages",
      schoolPackages: "School Packages",
      rentalPackages: "Rental Packages",
      systemHealth: "System Health",
      backups: "Backups",
      featureFlags: "Feature Flags",
      suggestions: "Suggestions",
      staffReviews: "Staff Reviews",
      notifications: "Notifications",
      auditLogs: "Audit Logs",
      settings: "Settings",
    },
    adminPortal: "Admin Portal",
    company: "TennaHub Technologies",
    logout: "Logout",
    managementPortal: "Management Portal",
    studentPortal: "Student Portal",
    teacherOnboardingSteps: [
      "Welcome",
      "Your Role",
      "Your Subjects",
      "Your Classes",
      "Match Subjects",
      "Done",
    ],
    teacherOnboardingTitles: [
      "Welcome to Your Teacher Portal",
      "What Type of Teacher Are You?",
      "Select Your Subjects",
      "Select Your Classes",
      "Match Subjects to Classes",
      "You're All Set!",
    ],
    teacherOnboardingDescriptions: [
      "Let's set up your profile. This takes just a minute.",
      "Choose how you'll be assigned to classes and subjects.",
      "",
      "Which classes will you be teaching?",
      "Tell us which subjects you teach in each class. Skip if you teach all subjects in all classes.",
      "Your profile is configured. You can always change these later.",
    ],
    levelLabels: {
      ecd: "ECD (Early Childhood)",
      primary: "Primary (P1-P7)",
      lower_secondary: "O-Level (S1-S4)",
      a_level: "A-Level (S5-S6)",
    },
    staffPortalLabels: {
      finance: "Finance",
      welfare: "Welfare",
      support: "Support",
      admissions: "Admissions",
      store: "Store",
      staff: "Staff",
    },
    portals: {
      headTeacher: "Head Teacher Portal",
      dos: "DOS Portal",
      teacher: "Teacher Portal",
      student: "Student Portal",
    },
    ecdOverrides: {
      students: "ECD Pupils",
      report_cards: "ECD Progress",
      gate_checkin: "Pupil Check-In",
    },
    moduleRoutes: {
      dashboard: "Dashboard",
      pos: "POS",
      products: "Products",
      sales: "Sales",
      customers: "Customers",
      employees: "Staff",
      expenses: "Expenses",
      reports: "Reports",
      settings: "Settings",
      menu: "Menu",
      tables: "Tables",
      qr_menu: "QR Menu",
      kitchen: "Kitchen",
      rooms: "Rooms",
      bookings: "Bookings",
      services: "Services",
      jobs: "Jobs",
      parts: "Spare Parts",
      appointments: "Appointments",
      prescriptions: "Prescriptions",
      patients: "Patients",
      internal_usage: "Internal Usage",
      suppliers: "Suppliers",
      categories: "Categories",
      stock_alerts: "Stock Alerts",
      purchase_orders: "Purchase Orders",
      business_cards: "Business Cards",
      payroll: "Payroll",
      accounting: "Accounting",
      students: "Students",
      classes: "Classes",
      attendance: "Attendance",
      gate_checkin: "Gate Check-in",
      visitor_register: "Visitor Register",
      grades: "Grades",
      subjects: "Subjects",
      fees: "Fees",
      academic_terms: "Academic Terms",
      inventory: "Inventory",
      letters: "Letters",
      student_cards: "Student Cards",
      term_requirements: "Term Requirements",
      report_cards: "Report Cards",
      parents: "Parents",
      assets: "Assets",
      ecd_pupils: "ECD Pupils",
      ecd_progress: "ECD Progress",
      ecd_roles: "ECD Roles",
      ecd_learning_areas: "ECD Learning Areas",
      ecd_pupil_cards: "ECD Pupil Cards",
      discipline: "Discipline",
      counseling: "Counseling",
      requisitions: "Requisitions",
      timetable: "Timetable",
      exams: "Exams",
      term_calendar: "Term Calendar",
      exam_sessions: "Exam Sessions",
      exam_results_import: "Import Exam Results",
      exam_import_permissions: "Exam Import Permissions",
      uneb_registration: "UNEB Registration",
      uneb_candidates: "UNEB Candidates",
      admission_links: "Admission Links",
      admission_confirmations: "Admission Confirmations",
      student_lifecycle: "Student Lifecycle",
      promotion_rules: "Promotion Rules",
      school_holidays: "School Holidays",
      uneb_exam_results: "UNEB Exam Results",
      exam_access: "Blocked Exam Access",
      marks_entry: "Marks Entry",
      academic_analytics: "Academic Analytics",
      rental_dashboard: "Rental Dashboard",
      rental_properties: "Properties",
      rental_units: "Units",
      rental_tenants: "Tenants",
      rental_leases: "Leases",
      rental_payments: "Payments",
      rental_maintenance: "Maintenance",
      rental_id_cards: "ID Cards",
      rental_payment_proofs: "Payment Proofs",
      suggestions: "Suggestions",
      staff_reviews: "Staff Reviews",
      notification_settings: "Notification Settings",
      notification_log: "Notification Log",
      parent_notification_preferences: "Parent Notification Preferences",
    },
  },

  // Messages
  messages: {
    toastTitles: [
      "Access Denied",
      "Account Created!",
      "Activity added",
      "Activity removed",
      "Added to Queue",
      "Admin Account Created!",
      "Admission link created",
      "Asset removed",
      "Booking cancelled",
      "Booking created",
      "Booking failed",
      "Branch Deleted",
      "Business Created!",
      "Card status updated",
      "Category added",
      "Category Deleted",
      "Category updated",
      "Class added successfully",
      "Class deleted successfully",
      "Class updated successfully",
      "Code copied to clipboard",
      "Copied!",
      "Current term updated",
      "Customer added",
      "Customer Created",
      "Customer Deleted",
      "Data exported",
      "Default subjects added",
      "Employee added",
      "Employee onboarded successfully",
      "Error",
      "Error adding item",
      "Error creating letter",
      "Error creating subject",
      "Error deleting letter",
      "Error deleting subject",
      "Error downloading cards",
      "Error saving attendance",
      "Error saving settings",
      "Error updating item",
      "Error updating letter",
      "Error updating subject",
      "Event booked successfully",
      "Event deleted",
      "Exam card downloaded",
      "Exam session created successfully",
      "Exam session deleted successfully",
      "Exam session updated successfully",
      "Expense recorded",
      "Expiry date set successfully",
      "Failed",
      "Failed to generate receipt image",
      "Failed to send receipt",
      "Features Updated",
      "Fee structure added successfully",
      "Fee structure deleted successfully",
      "Fee structure updated successfully",
      "Fees assigned successfully",
      "Footer image uploaded",
      "Header image uploaded",
      "Holiday deleted",
      "Holiday saved",
      "ID Card created successfully",
      "ID Card images downloaded",
      "Import Failed",
      "Invalid email or password",
      "Invalid school code",
      "Item added successfully",
      "Item removed successfully",
      "Item updated successfully",
      "Lease created successfully",
      "Lease marked as loss",
      "Lease updated successfully",
      "Letter created successfully",
      "Letter deleted successfully",
      "Letter updated successfully",
      "Link deleted",
      "Login Failed",
      "Maintenance request created",
      "Marked as Paid",
      "Menu item added",
      "Menu item deleted",
      "Menu item updated",
      "Missing Fields",
      "Missing Information",
      "No file selected",
      "No Students Selected",
      "Not Found",
      "Order failed",
      "Order placed!",
      "Part deleted",
      "Password Mismatch",
      "Password Reset",
      "Password reset email sent",
      "Password Too Short",
      "Patient Deleted",
      "Payment Approved",
      "Payment Collected",
      "Payment Failed",
      "Payment Proof Submitted!",
      "Payment Recorded",
      "Payment recorded successfully",
      "Payment Rejected",
      "Payment updated successfully",
      "Payment verified successfully",
      "Payroll Created",
      "PDF Downloaded",
      "Period deleted",
      "Permissions Removed",
      "PIN copied to clipboard",
      "Please enter a phone number",
      "Please fill all required fields",
      "Prescription Created",
      "Product Deleted",
      "Project added",
      "Project removed",
      "Property added successfully",
      "Property archived",
      "Property updated successfully",
      "Receipt downloaded!",
      "Rejection Failed",
      "Report card created",
      "Report card published",
      "Report card saved",
      "Request updated",
      "Requirement added successfully",
      "Requirement deleted successfully",
      "Requirement updated successfully",
      "Results saved",
      "Room added",
      "Rule deleted",
      "Rule saved",
      "SchoolPay settings saved!",
      "Select a table",
      "Select Session",
      "Service Deleted",
      "Session status updated",
      "Settings saved",
      "Settings saved successfully",
      "Settings Updated",
      "Signup Failed",
      "SMS app opened with receipt",
      "Staff Account Created",
      "Staff Updated",
      "Status updated",
      "Status updated successfully",
      "Stock limit reached",
      "Stock received",
      "Stock Updated",
      "Student account created",
      "Student Created",
      "Student deleted successfully",
      "Student enrolled successfully",
      "Student Not Found",
      "Student withdrawn",
      "Subject created successfully",
      "Subject deleted",
      "Subject deleted successfully",
      "Subject saved",
      "Subject updated successfully",
      "Submission Failed",
      "Subscription extended successfully",
      "Success",
      "Suggestion submitted",
      "Supplier Deleted",
      "Sync Complete",
      "Sync Failed",
      "Table added successfully",
      "Table deleted",
      "Table updated successfully",
      "Tenant added successfully",
      "Tenant Approved",
      "Tenant Deleted",
      "Tenant Rejected",
      "Tenant updated successfully",
      "Tenants Approved",
      "Tenants Rejected",
      "Term added successfully",
      "Term deleted successfully",
      "Term updated successfully",
      "Trial Updated",
      "Unauthorized",
      "Unit added successfully",
      "Unit archived",
      "Unit updated successfully",
      "Update Failed",
      "Updating...",
      "Upload failed",
      "Uploaded",
      "Verification Failed",
      "Verified",
      "Weak Password",
      "Webhook URL copied to clipboard",
      "Welcome back!",
      "WhatsApp opened with receipt",
      "You're up to date!",
    ],
    toastDescriptions: {
      accountCreated: "Account created successfully. Share credentials with the staff member.",
      adminWillReview: "Admin will review within 24-72 hours",
      allRowsErrors: "All rows have errors. Please check the data.",
      columnsAutoDetected: "Columns auto-detected from your Excel headers",
      credentialsCopied: "Credentials copied to clipboard",
      customerSaved: "Customer has been saved successfully",
      dataExported: "Data exported and records cleared",
      emailCopied: "Email copied to clipboard",
      examResultsNotFound: "Exam results not found",
      failedToGeneratePDF: "Failed to generate PDF. Please try again.",
      featureFlagsSaved: "Feature flags have been saved successfully",
      fullNameRequired: "Full name is required",
      imageTooLarge: "Image must be less than 2MB",
      linkCopied: "Link copied to clipboard",
      logoRemoved: "Logo removed successfully",
      logoUploaded: "Logo uploaded successfully",
      newCredentialsSent: "New credentials sent via WhatsApp",
      noUpdatesAvailable: "No new updates available.",
      noStudentFound: "No student found with that code or admission number.",
      noValidRows: "No valid rows to import",
      passwordsDoNotMatch: "Passwords do not match",
      patientSaved: "Patient record saved successfully",
      pleaseFillRequired: "Please fill all required fields",
      prescriptionUploaded: "Prescription uploaded for review",
      receiptSettingsSaved: "Receipt settings saved successfully",
      selfAdmissionUpdated: "Self-admission settings have been updated.",
      shareLink: "Share this link with the parent along with the payment code.",
      sponsorAdded: "Sponsor added successfully",
      sponsorDeleted: "Sponsor deleted successfully",
      sponsorUpdated: "Sponsor updated successfully",
      staffPermissionsSaved: "Staff permissions have been saved",
      tenantActivated: "Tenant account has been activated",
      appWillReload: "The app will reload with the latest version.",
      assetArchived: "The asset has been archived.",
      withdrawalUpdated: "Withdrawal settings have been updated",
      notAuthorizedAdmin: "You are not authorized to create an admin account.",
      youCanEnterScores: "You can now enter scores and remarks",
      youCanLogIn: "You can now log in as a platform administrator",
      noAdminPermissions: "You don't have admin permissions",
      noImportPermission: "You don't have permission to import exam results",
      joinedTeam: "You have successfully joined the team. Redirecting...",
      mustBeTenantOwner: "You must be a tenant owner to import data",
      mustBeLoggedIn: "You must be logged in",
      accountNotActive: "Your business account is not active yet",
      subscriptionExpiredMsg: "Your school subscription has expired. Please renew to continue.",
      loggedIn: "You've successfully logged in",
      loggedInAsAdmin: "You've successfully logged in as admin",
    },
  },

  // Business types
  businessTypes: {
    categories: {
      school: "Schools & Education",
      pharmacy: "Healthcare & Pharmacy",
      retail: "Retail & Shops",
      restaurant: "Restaurant, Bar & Cafe",
      hotel: "Hotels & Lodges",
      salon: "Salon & Spa",
      repair: "Repair & Workshop",
      rental: "Rental & Property Management",
      other: "Other",
    },
    types: {
      kindergarten: "Kindergarten/ECD",
      primary_school: "Primary School",
      secondary_school: "Secondary School",
      retail_shop: "Retail Shop",
      supermarket: "Supermarket",
      boutique: "Boutique",
      perfume_shop: "Perfume Shop",
      shoe_shop: "Shoe Shop",
      kitchenware_shop: "Kitchenware Shop",
      hardware: "Hardware Store",
      tech_shop: "Tech/Gadgets Shop",
      restaurant: "Restaurant",
      bar: "Bar",
      cafe: "Cafe",
      hotel: "Hotel",
      lodge: "Lodge",
      guest_house: "Guest House",
      salon: "Salon",
      spa: "Spa",
      barber: "Barber Shop",
      pharmacy: "Pharmacy",
      hospital: "Hospital/Clinic",
      clinic: "Clinic",
      garage: "Garage/Auto Workshop",
      tech_repair: "Phone Repair",
      car_spares: "Car Spare Parts",
      repair_shop: "General Repair Shop",
      rental_management: "Property Rental Management",
      property_management: "Property Management Company",
      other: "Other",
    },
  },

  // Components
  components: {
    common: {
      buttons: {
        save: "Save",
        cancel: "Cancel",
        add: "Add",
        edit: "Edit",
        delete: "Delete",
        submit: "Submit",
        close: "Close",
        confirm: "Confirm",
        back: "Back",
        next: "Next",
        print: "Print",
        download: "Download",
        upload: "Upload",
        refresh: "Refresh",
        search: "Search",
        filter: "Filter",
        export: "Export",
        import: "Import",
        view: "View",
        activate: "Activate",
        deactivate: "Deactivate",
      },
      labels: {
        name: "Name",
        email: "Email",
        phone: "Phone",
        address: "Address",
        status: "Status",
        date: "Date",
        type: "Type",
        category: "Category",
        amount: "Amount",
        quantity: "Quantity",
        price: "Price",
        description: "Description",
        notes: "Notes",
        total: "Total",
        active: "Active",
        inactive: "Inactive",
      },
      pagination: {
        previous: "Go to previous page",
        next: "Go to next page",
        pagination: "pagination",
      },
      sidebar: {
        toggle: "Toggle Sidebar",
      },
      breadcrumb: "breadcrumb",
      logout: "Logout",
      networkStatus: {
        offline: "You are offline",
        online: "Back online",
      },
      installPrompt: {
        install: "Install App",
        description: "Install TennaHub for a better experience",
      },
      keyboardShortcuts: "Keyboard Shortcuts",
    },
  },

  // Signup categories
  signupCategories: {
    education: {
      title: "Staff & Teacher Sign-Up",
      subtitle: "School accounts are created by your administrator. Staff and teachers are invited via email.",
      heading: "Join Your School",
      invitationDesc: "If your school administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your school dashboard, manage students, and more.",
      adminTitle: "Create a school account",
      adminDesc: "If you are a school owner or administrator, you can register your school directly.",
      adminAction: "Register your school here",
    },
    healthcare: {
      title: "Staff Sign-Up",
      subtitle: "Pharmacy accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Pharmacy",
      invitationDesc: "If your pharmacy administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your pharmacy dashboard, manage inventory, and more.",
      adminTitle: "Register your pharmacy",
      adminDesc: "If you are a pharmacy owner or administrator, you can register your pharmacy directly.",
      adminAction: "Register your pharmacy here",
    },
    retail: {
      title: "Staff Sign-Up",
      subtitle: "Store accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Store",
      invitationDesc: "If your store administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your store dashboard, manage sales, and more.",
      adminTitle: "Register your store",
      adminDesc: "If you are a store owner or administrator, you can register your store directly.",
      adminAction: "Register your store here",
    },
    restaurant: {
      title: "Staff Sign-Up",
      subtitle: "Restaurant accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Restaurant",
      invitationDesc: "If your restaurant administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your restaurant dashboard, manage orders, and more.",
      adminTitle: "Register your restaurant",
      adminDesc: "If you are a restaurant owner or administrator, you can register your restaurant directly.",
      adminAction: "Register your restaurant here",
    },
    hotel: {
      title: "Staff Sign-Up",
      subtitle: "Hotel accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Hotel",
      invitationDesc: "If your hotel administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your hotel dashboard, manage bookings, and more.",
      adminTitle: "Register your hotel",
      adminDesc: "If you are a hotel owner or administrator, you can register your hotel directly.",
      adminAction: "Register your hotel here",
    },
    salon: {
      title: "Staff Sign-Up",
      subtitle: "Salon accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Salon",
      invitationDesc: "If your salon administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your salon dashboard, manage appointments, and more.",
      adminTitle: "Register your salon",
      adminDesc: "If you are a salon owner or administrator, you can register your salon directly.",
      adminAction: "Register your salon here",
    },
    repair: {
      title: "Staff Sign-Up",
      subtitle: "Workshop accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Workshop",
      invitationDesc: "If your workshop administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your workshop dashboard, manage jobs, and more.",
      adminTitle: "Register your workshop",
      adminDesc: "If you are a workshop owner or administrator, you can register your workshop directly.",
      adminAction: "Register your workshop here",
    },
    other: {
      title: "Staff Sign-Up",
      subtitle: "Business accounts are created by your administrator. Staff members are invited via email.",
      heading: "Join Your Business",
      invitationDesc: "If your business administrator sent you an invitation email, click here to accept it and create your account.",
      signInDesc: "Sign in to access your dashboard and manage your business.",
      adminTitle: "Register your business",
      adminDesc: "If you are a business owner or administrator, you can register your business directly.",
      adminAction: "Register your business here",
    },
  },

  // Welcome onboarding
  welcomeOnboarding: {
    welcome: "Welcome to TennaHub",
    tagline: "The all-in-one platform to run your business like a pro. Let's get you started!",
    getStarted: "Get Started",
    justBrowse: "I'll just browse",
    madeForUganda: "Made for Uganda",
    businessQuestion: "What type of business do you run?",
    personalize: "We'll personalize your experience",
    continueWith: "Continue with",
    howProceed: "How would you like to proceed?",
    createOrSignIn: "Create a new account or sign in to your existing one",
    createNewAccount: "Create New Account",
    signInExisting: "Sign In to Existing Account",
    skipToHome: "Skip - take me to the homepage",
    back: "Back",
    businessOptions: {
      education: {
        title: "Schools & Education",
        description: "Kindergartens, Primary & Secondary Schools",
      },
      retail: {
        title: "Retail & Shops",
        description: "Supermarkets, Boutiques, Hardware",
      },
      restaurant: {
        title: "Restaurants & Bars",
        description: "Restaurants, Cafes, Bars & Food",
      },
      hotel: {
        title: "Hotels & Lodges",
        description: "Hotels, Lodges & Guest Houses",
      },
      salon: {
        title: "Salons & Spas",
        description: "Beauty Salons, Spas & Barbers",
      },
      healthcare: {
        title: "Healthcare",
        description: "Pharmacies, Clinics & Hospitals",
      },
      repair: {
        title: "Repair Services",
        description: "Garages, Phone Repair & Workshops",
      },
      other: {
        title: "Other Business",
        description: "Any other business type",
      },
    },
  },
};

export type TranslationKeys = typeof en;
