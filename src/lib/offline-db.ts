...
interface PharmacyDBSchema extends DBSchema {
  "pending-sync": {
    key: string;
    value: {
      id: string;
...
let dbPromise: Promise<IDBPDatabase<PharmacyDBSchema>> | null = null;

const getDB = () => {
  if (!dbPromise) {
    dbPromise = openDB<PharmacyDBSchema>("tennahub-offline", 2, {
      upgrade(db) {
